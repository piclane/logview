...
public class Json {
	/**
	 * JSON をバイト配列からオブジェクトを生成します
	 * 
	 * @param bytes JSON を指定されたエンコーディングで表現したバイト配列
	 * @param clazz 変換先オブジェクトのクラス
	 * @param encoding エンコードに使用された文字セット名
	 * @return 変換されたオブジェクト
	 */
	public static <T> T deserialize(byte[] bytes, Class<T> clazz, String encoding) {
		return deserialize(new ByteArrayInputStream(bytes), clazz, encoding);
	}
	
	/**
	 * JSON をストリームからオブジェクトを生成します
	 * 
	 * @param is JSON を指定されたエンコーディングで表現したストリーム
	 * @param clazz 変換先オブジェクトのクラス
	 * @param encoding エンコードに使用された文字セット名
	 * @return 変換されたオブジェクト
	 */
	public static <T> T deserialize(InputStream is, Class<T> clazz, String encoding) {
		Charset charset = StandardCharsets.UTF_8;
		if(encoding != null && !encoding.isEmpty()) {
			charset = Charset.forName(encoding);
		}
		
		return deserialize(new InputStreamReader(is, charset), clazz);
	}
